...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////opt/lewis/app/lewis_ai.db'
...
CHINESE_ZODIAC = {
    ...
    1995: '豬', 1996: '鼠', 1997: '牛', 1998: '虎', 1999: '兔',
    2000: '龍', 2001: '蛇', 2002: '馬', 2003: '羊', 2004: '猴',
    2005: '雞', 2006: '狗', 2007: '豬', 2008: '鼠', 2009: '牛'
}
...
def web_static(filename):
    return send_from_directory(f'{BASE_DIR}/web', filename)
...
@login_required
def dashboard():
    fortune_count = FortuneRecord.query.filter_by(user_id=current_user.id).count()
    music_count = MusicRecord.query.filter_by(user_id=current_user.id).count()
...
def fortune():
    ...
    if request.method == 'POST':
        ...
        month_day = birth_date[5:]
        
        result = {
            'zodiac': get_zodiac(month_day),
...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <div class="stats">
                <div class="stat"><div class="stat-num">{{ fortune_count }}</div><div class="stat-label">命理分析</div></div>
                <div class="stat"><div class="stat-num">{{ music_count }}</div><div class="stat-label">AI 音樂</div></div>
                <div class="stat"><div class="stat-num">{{ agent_count }}</div><div class="stat-label">代理推薦</div></div>
...
if __name__ == '__main__':
    os.makedirs('/opt/lewis/app', exist_ok=True)
